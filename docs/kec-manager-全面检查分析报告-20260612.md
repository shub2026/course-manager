...
| 编号 | 问题 | 状态 | 详情 |
|------|------|------|------|
| **M1** | 路由错误处理模式不一致（3 种方式） | ✅ **已修复** | 经核实所有路由已统一使用模式C（try/catch + next），包括auth/user/audit等16个路由文件 |
...
| 维度 | 严重 | 高危 | 中危 | 低危 | 合计 |
|------|------|------|------|------|------|
| 后端安全 | 5（5已修复） | 8（7已修复） | 12（11已修复） | 12 | 37 |
...
| 状态 | 数量 | 占比 | 编号 |
|------|------|------|------|
| ✅ 已修复 | 24 | 36.4% | C1, C2, C3, C4, C5, H1, H5, H6, H7, H8, M1, M2, M3, M4, M5, M7, M8, M9, M10, M11, M12, sortOrder命名, update-class-status.js, FC1/FC3/FH2 |
